...
class _LineChartSample2State extends State<LineChartSample2> {
  ...
  double _wakeUpSlider = 50;
  double _sleepSlider = 50;
                  ...
                  Expanded(
                    child: SliderTheme(
                      data: SliderTheme.of(context).copyWith(
                        trackShape:
                            const RectangularSliderTrackShape(), // 원형 대신 직선 트랙
                        trackHeight: 4.0,
                        thumbShape: const RoundSliderThumbShape(
                          enabledThumbRadius: 8,
                        ),
                        overlayShape:
                            SliderComponentShape.noOverlay, // 터치시 퍼지는 원 제거
                        // trackLeftPadding 제거는 직접 사이즈를 제한하는 게 더 효과적
                      ),
                      child: Slider(
                        value: _wakeUpSlider,
                        min: 0,
                        max: 100,
                        divisions: 100,
                        label: _wakeUpSlider.toInt().toString(),
                        activeColor: Colors.grey[200], // 슬라이더 트랙 채워진 쪽 색
                        inactiveColor: Colors.white24, // 트랙 비어있는 쪽 색
                        thumbColor:
                            Colors.grey[200], // Thumb 색 (Flutter 3.7 이상부터)
                        onChanged: (val) {
                          setState(() {
                            _wakeUpSlider = val;
                          });
